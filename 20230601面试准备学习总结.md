...
#### 26.【疑问·探索】某种不走浏览器下载管理器的下载处理方式？

回答来自于chatgpt3.5

在前端中，您可以通过以下方式实现下载不走浏览器下载任务，同时实现实时下载进度和保存文件到本地：

1. **使用 Fetch API 和 Response Body Stream：** 您可以使用 Fetch API 获取文件的响应流，然后通过分段读取流中的数据来模拟下载过程。您可以通过监听流的 `data` 事件来获取实时的下载进度，然后将数据保存到本地。请注意，这种方式不会触发浏览器的下载任务，但您需要自己处理下载进度和保存逻辑。
2. **使用 Web Workers：** Web Workers 允许在后台线程中执行 JavaScript 代码，您可以在 Web Worker 中执行文件的下载和保存操作，同时在主线程中获取实时的下载进度。这样可以确保下载不影响主线程的性能，并提供实时的下载进度反馈。
3. **使用 Blob 和 URL.createObjectURL：** 您可以将下载的数据创建成 Blob 对象，然后通过使用 URL.createObjectURL 创建一个临时的下载链接，将链接指向 Blob，这样用户可以点击链接进行下载。这不会触发浏览器的下载任务，但可以让用户手动点击链接下载文件。
4. **使用 IndexedDB：** 如果需要将下载好的文件保存在浏览器本地，您可以使用 IndexedDB 进行本地存储。您可以将下载的数据存储在 IndexedDB 中，并在需要时将数据提取出来保存到用户本地。

请注意，上述方法可能需要复杂的前端代码来实现，并且在不同浏览器中可能会有一些限制和差异。此外，这些方法可能需要一些安全性的考虑，因为在处理文件和下载时涉及到用户的隐私和安全问题。最好的做法是仔细考虑用户体验和安全性，确保您的实现符合最佳实践。

[评语] 感觉chat是回答偏题了，试了几版提问方式都不得要领，我大概描述一下原现象：

用谷歌浏览器在某个网站上下载东西，但是没有触发谷歌浏览器的下载管理器，而是看起来"在网页里下载"（页面上会实时展示进度，但是浏览器的下载管理器中没有该任务）。进度下满后，可以点击按钮直接将已经下载好的文件保存在本地。不知道和http/3是否有关系。
