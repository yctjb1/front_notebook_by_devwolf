...
解析：宏任务（如 `setTimeout`、`Promise` 的构造函数）和微任务（`Promise` 的 `then` 方法、process.nextTick）

【重点！】**事件循环的执行过程**——事件循环的执行过程可以简化为以下步骤：
...
5. 在宏任务执行期间，如果产生了微任务，这些微任务会被添加到微任务队列中，等待当前宏任务执行结束后执行。等微任务都结束后，才会执行下一个宏任务
...
拦截器分为两类：请求拦截器（Request Interceptors）和响应拦截器（Response Interceptors）。
...
.box {
    ...
    /* border-box下，width是多少盒子就占多少宽。盒子所占宽200 = 10*2 + 20 *2 + 140 */
}
 .box2 {
    width: 200px;
    height: 100px;
